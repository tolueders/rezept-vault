...
export function RecipeSearch({ categories, initialRecipes = [] }: RecipeSearchProps) {
  ...
  const doSearch = useCallback(async (q: string, cat: string) => {
    ...
    setLoading(true);
    try {
      const data = await searchRecipesAction(q, cat === "all" ? undefined : cat);
      setResults(data);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <p className="text-center text-muted-foreground">Suche…</p>
      ) : results.length === 0 ? (
        <p className="py-12 text-center text-muted-foreground">
          {query ? "Keine Rezepte gefunden" : "Noch keine Rezepte vorhanden"}
        </p>
      ) : (
